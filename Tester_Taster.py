...
# Confirm button
confirm_btn = pygame.Rect(558, 565, 100, 50)
...
confirm_btn_enabled = True
running = True
while running:
    screen.fill('White')
    pygame.draw.rect(screen,'black',btn1)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            if confirm_btn.collidepoint(event.pos) and confirm_btn_enabled:
                show_img = True
                print('btn clicked')
    ...
    if confirm_btn.collidepoint(pygame.mouse.get_pos()):
        hover=True
    else:
        hover=False

    if hover:
        pygame.draw.rect(screen, hover_color, confirm_btn)
    else:
        pygame.draw.rect(screen, button_color, confirm_btn)
    ...
    if confirm_btn_press_cnt>=2:
        ...
        confirm_btn_txt='PROCEED'
    
    screen.blit(font.render(confirm_btn_txt,True,'black'),(562, 577))
